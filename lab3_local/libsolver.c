...
// main 在執行時的位址（透過 game_get_ptr("main") 取得）
static uintptr_t main_runtime_addr = 0;
static uintptr_t main_symbol_offset = 0x16c89; // 根據你自己的 binary 設定

// 設定 main 函式的執行位址，初始化時呼叫一次
void set_main_address(uintptr_t runtime_addr) {
  main_runtime_addr = runtime_addr;
}

// 幫助你找出第 N 個指令對應的 GOT 表 entry 實體位置
