...
int b64decode(const char *input, unsigned char *output, int outlen) {
  ...
  for (i = 0; i < len && olen < outlen; i += 4) {
    unsigned int t = 0;
    for (j = 0; j < 4; j++)
      t |= (c2v(input[i + j]) % 64) << (18 - 6 * j);
    for (j = 0; j < 3 && olen < outlen; j++)
      output[olen++] = (t >> (16 - 8 * j)) & 0x0ff;
  }
  return olen;
}

int /* 0: ok; -1: error or empty request */
parse_request(req_t *req) {
  int err = 0;
...
#define DELIM " \t\n\r"
  ...
  if (fgets(line, sizeof(line), stdin) == NULL)
    return -1;
  if ((tok = strtok_r(line, DELIM, &saveptr)) == NULL)
    return -1;
  if (strcmp(tok, "GET") != 0) {
    err = -1;
    goto headers;
  ...
  while (fgets(line, sizeof(line), stdin) != NULL) {
    ...
    if (strcmp(tok, "Authorization:") == 0) { // 'Auth' header
      ...
      if (strcmp(tok, "Basic") != 0)
        continue; // not 'Basic Auth'
      if ((tok = strtok_r(NULL, DELIM, &saveptr)) == NULL)
        continue;
      if (b64decode(tok, (unsigned char *)password, _REQ_PASSLEN - 1) < 0)
        continue;
      if (strncmp(password, "admin:", 6) != 0)
...
static char *__load_file_internal(int fd, int sz) {
  ...
  if ((wptr = content = (char *)malloc(sz)) == NULL)
    goto quit;
...
char *load_file(const char *fname, int *sz) {
  ...
  *sz = 0;
  debug("[load_file][tid=%lu] opening file: %s\n", (unsigned long)tid, fname);

  if ((fd = open(fname, O_RDONLY)) < 0) {
    debug("[load_file][tid=%lu] open() failed: %s\n", (unsigned long)tid,
          strerror(errno));
    return NULL;
  }
  ...
  if (stat(fname, &st) < 0) {
    debug("[load_file][tid=%lu] stat() failed: %s\n", (unsigned long)tid,
          strerror(errno));
    goto quit;
  }

  debug("[load_file][tid=%lu] stat size = %ld\n", (unsigned long)tid,
        st.st_size);
  ...
  *sz = st.st_size;
  ...
  if (content) {
    int preview_len = (*sz < 64) ? *sz : 64;
    debug("[load_file][tid=%lu] read %d bytes, preview: ", (unsigned long)tid,
          *sz);
    for (int i = 0; i < preview_len; i++) {
      debug("%c", isprint(content[i]) ? content[i] : '.');
    }
    debug("\n");
  } else {
    debug("[load_file][tid=%lu] content is NULL (malloc/read failed)\n",
          (unsigned long)tid);
  }
...
int /* 0: pass; 1: fail = auth required */
...
int /* -1: error; 0: pass; 1: auth required */
check_path(const char *path, const char *password, unsigned long cookie) {
  char rpath[PATH_MAX];
  if (realpath(path, rpath) == NULL)
    return -1;
  if (strncmp(root, rpath, strlen(root)) != 0)
    return -1;
...
void *handle_request(void *arg) {
  ...
  if (rsp.need_auth) {
    fprintf(stdout,
            "WWW-Authenticate: Basic realm=\"Secret of the Challenge\"\r\n");
    fprintf(stdout,
            "Set-Cookie: challenge=%u; Path=/; Max-Age=3600; HttpOnly; "
            "SameSite=Strict\r\n",
            reqseed);
  }
