...
void do_job(long id, string server) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    snprintf(buf, sizeof(buf), "Connecting to %u.%u.%u.%u:%u ... (%d)",
             NIPQUAD(sin.sin_addr.s_addr), ntohs(sin.sin_port), i + 1);
    ...
    if (connect(s, (struct sockaddr *)&sin, sizeof(sin)) < 0)
      goto try_again;
    if (write(s, cmd, sizeof(cmd) - 1) < 0)
      goto try_again;
    if ((rlen = read(s, buf, sizeof(buf) - 1)) <= 0)
      goto try_again;
...
int main() {
  ...
  if ((errno = pthread_create(&tid, NULL, threadproc, (void *)0)) != 0)
    xerror("thread");
  if ((errno = pthread_create(&tid, NULL, threadproc, (void *)1)) != 0)
    xerror("thread");
